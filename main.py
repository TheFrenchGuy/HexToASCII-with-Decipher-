...
letterPercentage = dict(zip(string.ascii_uppercase,
...
def percentage(msg):  ##So that it returns the most english message (HAHAHAH so funny because its me)
    return sum(letterPercentage.get(char, 0) for char in msg)  ##Evalutate all of the char each by one
